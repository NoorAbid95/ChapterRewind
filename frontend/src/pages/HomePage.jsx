...
import bookFlip from "../assets/book-flip.png";
import "./HomePage.css";
...
const HomePage = () => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      navigate("/search"); //Found that navigate doesn't reload the whole page which would have happened if I made form action='/search'
    } catch (error) {
  ...
  return (
    <div>
      <div className="flex items-center justify-center min-h-screen py-10 transition-all duration-700  ">
        <div
          className="w-full max-w-2xl flex flex-row items-center justify-center gap-10 px-8 py-8 rounded-3xl bg-black/10 transition-all duration-700"
        >
